...
int main(int argc, char* argv[]){
	...
	int j;
	float n1,n2,n3;
	int vetorRa[atoi(argv[1])],w,t,q,temp;
	FILE* arq, *indices;
	...
	//cria vetor tamanho qtReg em ordem, para ser embaralhado
	for(q=0;q<qtReg;q++){
		vetorRa[q]=q;
	}
	...
	if (qtReg > 1) {
        ...
        for (w = 0; w < qtReg - w; w++) {                            //Embaralha os elementos do vetor de registros para gerar uma 
            t = w + rand() / (RAND_MAX/(qtReg-w)+1);                 //sequência de inteiros aleatória
            temp = vetorRa[t];
            vetorRa[t] = vetorRa[w];
            vetorRa[w] = temp;
        }
